...
| Parameter | Important for Access Token Control? | Enforce Now (V2)? | Accepted Values / Rule | Justification |
|----------|-------------------------------------|-------------------|------------------------|---------------|
...
| **Key (`<PublicKey>` / `<SecretKey>`)** | ✅ Critical | ✅ Yes (**ERROR**) | Must be present and consistent with Algorithm: HS* → SecretKey, RS*/PS*/ES* → PublicKey (Value, Certificate, or JWKS). | Without a key, no cryptographic verification is performed. This ensures the token is actually signed by a trusted authority. |
...
| **Key Source (`<PublicKey><Value>`)** | ⚠️ Medium | ⚠️ Yes (**WARNING**) | Should use `ref=` (KVM or variable), avoid hardcoded values. | Hardcoded keys expose sensitive data in source code and reduce security. |
| **Expiration (`exp` claim)** | ✅ High | ❌ No | Automatically enforced by Apigee VerifyJWT. | No need to enforce in plugin; already handled by the runtime. |
| **Not Before (`nbf` claim)** | ⚠️ Medium | ❌ No | Automatically handled if present. | Managed by Apigee; not required for plugin enforcement. |
...
| **JWKS Advanced Handling (`<JWKS>`)** | ⚠️ Medium | ❌ No (V3) | Basic presence accepted | Advanced key rotation logic adds complexity; deferred. |
